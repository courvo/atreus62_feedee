...
washer_radius     = 3 * screw_hole_radius;
...
module right_screw_holes(hole_radius) {
  ...
  translate([washer_radius - tmp[0] - 0.5*hand_separation+10,
...
module quartered_spacer(explode)
{
  /* Assemble all four quarters of a spacer. */
  translate([explode,explode]) spacer_quadrant(0);
  translate([-explode,explode]) spacer_quadrant(1);
  translate([-explode*1.25,-explode]) spacer_quadrant(2);
  translate([explode*1.25,-explode]) spacer_quadrant(3);
}

/* Create all four layers. */
switch_plate();
quartered_spacer(explode = 15);


translate([0, 150]) bottom_plate();

translate([300, 120]) rotate(180) top_plate();;
